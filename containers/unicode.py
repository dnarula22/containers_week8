...
class NormalizedStr:
    ...
    def __repr__(self):
        ...
        '''
        helper = unicodedata.normalize(self.normal_form, self.text)
        return "NormalizedStr('" + helper + "', '" + self.normal_form + "')"
    ...
    def __add__(self, b):
        ...
        '''
        normal_self = unicodedata.normalize(self.normal_form, str(self.text))
        normal_b = unicodedata.normalize(self.normal_form, str(b))
        selfb = normal_self + normal_b
        normal_selfb = unicodedata.normalize(self.normal_form, selfb)
        return NormalizedStr(normal_selfb)
    ...
    def __iter__(self):
        ...
        '''
        return NormalStrIter(self.__str__())

class NormalStrIter:

    def __init__(self, text, m=-1):
        self.text = text
        self.m = m

    def __next__(self):
        if self.m == (len(self.text) - 1):
            raise StopIteration
        else:
            self.m += 1
            return self.text[self.m]
